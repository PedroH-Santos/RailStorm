...
- **DOTween** (`Assets/Plugins/Demigiant/DOTween/`) — lib de animação por tween, adotada a partir de 14/09 para UI (roleta/faíscas do baú, seção 4.15) no lugar de corrotinas com `Systems/Easing.cs`. Toda tela que roda pausada (`Time.timeScale = 0`) precisa de `.SetUpdate(true)` em cada tween/sequência, e matar (`DOKill`)/`SetLink(gameObject)` ao fechar/desativar. `Systems/Easing.cs` está em descontinuação — não usar em código novo, mas os consumidores existentes (`ChooseWayScreenAnimator`, `SplineUnlockSequence`, etc.) não são migrados automaticamente.
...
**O que é / ideia central:** é a tela que aparece ao abrir um baú, no mesmo vocabulário cartoon "madeira pintada" das telas de escolha de caminho e do card de habilidade — um cartão de madeira central com uma pequena roleta que troca de item cada vez mais devagar até parar no item sorteado, cercada de faíscas subindo na cor da raridade, para a tela parecer viva mesmo antes do jogador decidir Pegar/Exilar/Pular.
...
- A troca de item na roleta acelera→desacelera: intervalo entre trocas começa em `tickStart` e cresce por `tickGrowth` a cada troca até passar de `tickEnd` (~2,5s no total). **Sem opção de pular** — os 3 botões ficam `interactable = false` até o último tick.
- O último tick mostra sempre o item realmente sorteado, nunca um resultado diferente do decidido antes.
- Cada tick pinta a borda do cartão, a placa de raridade e a cor das faíscas na raridade do item daquele instante (mesmo em itens intermediários do sorteio), então a moldura "pisca" raridades durante a troca.
- Toda a sequência roda em `Time.unscaledDeltaTime`/tweens com `SetUpdate(true)` — o jogo fica pausado (`Time.timeScale = 0`) igual às outras telas de decisão.
- Painéis laterais de Inventário e Status são **duplicados** da tela de seleção de habilidade (`CanvasSkillSelector`), mas reestilizados em madeira **só nesta tela** — a tela de habilidades continua no tema navy/`OrnateFrame9Slice` original. As duas telas compartilham os mesmos scripts (`InventoryUI`, `InventorySection`, `StatsUI`) e o mesmo slot de arma/skill/item (`WeaponUI.prefab`); só a pele muda.

**Anatomia do `ChestPanel`** (cena `CanvasEventChestItem/SkillSelectionPanel/ChestPanel`, madeira igual ao `BadgeCanvas`/`BottomBar` da seção 4.13 — `SignShadow9Slice`→`SignPlate9Slice`→`SignBorder9Slice`, a borda tintada pela raridade em runtime):
- `Content/RarityTag` — placa (`AccentPlate9Slice`) com o nome da raridade, cor de raridade em runtime.
- `Content/Title` / `Content/Description` — nome e descrição do item; mostram `"Sorteando..."`/vazio enquanto a roleta gira.
- `Content/RevealWindow` — janela em **madeira escura rebaixada** (não navy) que abriga a roleta: `Sparks` (emissor de faíscas) atrás, `RevealSlot` (placa de raridade + ícone do item) no centro. Ver "Janela central" abaixo.
- `Content/ButtonsRow` — `ExileButton`/`SkipButton`/`TakeButton` (`ButtonPlate9Slice`, cores `actionDestructive`/`actionNeutral`/`actionPrimary`), travados durante a troca.
- Cor/fonte/sprite/tamanho de cada peça são configurados **na cena**, não em código — só o que depende do item sorteado (ícone, sprite/cor de placa por raridade, textos) é escrito em runtime.

**Painéis laterais em madeira (14/09, ajuste pós-feedback).** A primeira versão deixou Inventário/Status no `OrnateFrame9Slice` navy da tela de habilidades, destoando do `ChestPanel` de madeira ao lado — a queixa do usuário foi direta ("não estão muito combinado"). Correção: cada painel (`InventoryPanel`, `StatsPanel`) ganhou os mesmos três filhos do `ChestPanel` — `WoodShadow`/`WoodFill`/`WoodBorder` (`SignShadow9Slice`→`SignPlate9Slice`→`SignBorder9Slice` em `woodOutline`, `LayoutElement.ignoreLayout = true`) — e a `Image` original (`OrnateFrame9Slice`) foi **desativada** (`enabled = false`, não removida, pra não perder a referência caso a tela de habilidades precise dela de volta). Títulos e textos de seção passaram de creme para `woodOutline` (texto sobre madeira, mesma regra da seção 4.13); os cabeçalhos de grupo do Status (`VITAL`/`ATRIBUTOS`/`RECURSOS`) foram de navy (`#112942`) para Wood Mid (`#A0754B`) com `Shadow` no título; a barra de rolagem foi de navy/Copper para Wood Dark/Copper. O `padding` do `VerticalLayoutGroup` de cada painel subiu de 34 para 48, pra acompanhar a borda mais grossa do `SignBorder9Slice` (mesma regra "padding ≥ espessura da borda" da seção 4.10/4.13).

A linha de status ganhou uma segunda pele: **`Assets/Prefabs/UI/StatsWood.prefab`** — cópia de `Stats.prefab` com o fundo trocado para `SignPlate9Slice` em Wood Light (`#DEAD8B`) e texto em `woodOutline`, ligada só no `StatsUI.statRowPrefab` do baú (`Stats.prefab` original continua servindo a tela de habilidades, sem mudança de comportamento). Para isso, [StatRowUI.cs](Assets/Scripts/UI/AbilitiesUI/Stats/StatRowUI.cs) ganhou um campo `applyTheme` (`true` por padrão): quando ligado, `Setup` continua chamando `theme.ApplyStatLabel`/`ApplyStatValue` (Steel Light/creme, como sempre); a variante de madeira desliga a flag no prefab, e cor/fonte ficam só o que já está gravado na cena — sem isso, o texto do `UIThemeConfig` (claro) ficaria ilegível sobre a madeira clara do `StatsWood`.

**Janela central (14/09, ajuste pós-feedback).** A primeira versão usava `SolidRounded9Slice` navy (cantos arredondados) com um `SignBorder9Slice` chanfrado por cima — a forma do fundo não batia com a da borda, e o navy ficava "estranho"/pouco visível encostado na madeira do painel. Correção: o fundo da `RevealWindow` passou a ser o próprio `SignPlate9Slice`, tintado em Wood Dark (`#724E21`) — mesma silhueta chanfrada da borda, então as duas casam exatamente. O recorte de conteúdo trocou de `RectMask2D` (retangular — deixava faísca vazar nas quinas chanfradas) para `Mask` com `showMaskGraphic = true`, usando essa mesma `Image` como máscara: o recorte agora segue o chanfro. O `Border` (`SignBorder9Slice`) foi movido pra **fora** da janela, virando irmão dela dentro de `Content` com o mesmo retângulo (`anchorMin`/`anchorMax`/`sizeDelta`/`anchoredPosition` copiados) e `LayoutElement.ignoreLayout = true` — um `Mask` corta todos os filhos da hierarquia mascarada, então um contorno teria sido cortado nas quinas se continuasse dentro.

**Faíscas até o topo (14/09, ajuste pós-feedback).** [ChestSparksEmitter.cs](Assets/Scripts/Events/Chest/ChestSparksEmitter.cs) tinha um `rise` fixo (`220`) menor que a altura real da `RevealWindow` (`520`), então a faísca desaparecia (fade-out nos 30% finais da vida) bem antes de chegar ao topo. Virou `riseFraction` (`Range 0.5–1.5`, padrão `1.1`): a subida agora é `sparksRoot.rect.height * riseFraction`, sempre relativa ao tamanho real da janela — se a janela for redimensionada depois, a faísca continua saindo pela borda de cima. A faísca também passou a nascer um pouco abaixo da base (`-height/2 - sparkHeight`), pra entrar em cena em vez de já aparecer parcialmente visível dentro da janela.

**Scripts:**
- **`ChestRevealEffect.cs`** — orquestrador (singleton, padrão de auto-ativação da seção 3). `Show(item, rarityIndex, reelPool, worldPos, onTake, onExile, onSkip)`: prepara textos pendentes, ativa o painel (tween de escala `DOTween`, `Ease.OutBack`, `SetUpdate(true)`) e dispara a roleta; só libera os botões e mostra o resultado final no `onComplete` do `ChestRouletteAnimator`. `Decide` ignora cliques enquanto a roleta gira. `Close` anima saída (`Ease.InCubic`) e o dim (`gameBackground`) via `DOFade`.
- **`ChestRouletteAnimator.cs`** — a troca em si: monta uma `DOTween.Sequence()` de callbacks + intervalos crescentes (`tickStart`×`tickGrowth` até `tickEnd`), cada tick chama `ApplyTick` (ícone/placa/borda/faíscas), termina em `Land` (item final + punch maior + rajada de faíscas). `Stop()` mata a sequência (usado ao fechar a tela).
- **`ChestSparksEmitter.cs`** — pool de `Image` (`sparkPoolSize`) recicladas; cada faísca emitida ganha uma `Sequence` DOTween (sobe + deriva lateral + rotação + fade), devolvida ao pool no `OnComplete`. `SetSpinning`/`Burst` controlam o ritmo (mais rápido durante a troca, rajada ao pousar, ritmo baixo depois).
- **`ChestInteractable.cs`** — monta o `reelPool` (itens da loot table não exilados/possuídos, garantindo que o item sorteado esteja incluso) e repassa pro `Show`.
- **`Assets/Prefabs/UI/StatsWood.prefab`** — cópia de `Stats.prefab` na paleta de madeira, usada só pelo `StatsUI.statRowPrefab` do painel de Status do baú (ver "Painéis laterais em madeira" acima).

> **`SignBorder9Slice` só alinha com `SignPlate9Slice`, não com `SolidRounded9Slice`.** As duas primeiras vêm da mesma família chanfrada (mesmo raio de canto); a `SolidRounded9Slice` é arredondada — sobrepor o anel chanfrado num fundo arredondado deixa o contorno visivelmente descolado do fundo nas quinas. Ao criar uma peça nova com borda + preenchimento separados, usar sempre sprites da mesma família de cantos.

> **DOTween em vez de `Systems/Easing.cs`.** Esta tela é a primeira a usar DOTween no lugar de corrotinas com `Easing` (que o projeto está descontinuando — ver seção 2). Todo tween usa `.SetUpdate(true)` (a tela roda pausada) e é morto explicitamente (`DOKill`) ao fechar/desativar, para nenhuma animação continuar rodando com o painel escondido.
