...
public class PerkCardUI : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void Setup(PerkCardData data, Action onClick)
    {
        _onClick = onClick;
        SetSelected(false);
        ...
        if (d is PerkDefinition perk)
        {
            descriptionText.text = perk.description;
        }
        else if (d is WeaponPerkDefinition weaponPerk)
        {
            var level = weaponPerk.GetLevelForRarity(ri);
            descriptionText.text = !string.IsNullOrEmpty(weaponPerk.description)
                ? weaponPerk.description
                : $"{StatLabels.Of(weaponPerk.statTarget)} +{level.statValue:0.#}{(level.isMultiplier ? "%" : string.Empty)}";
        }
        else if (d is CarWeaponDefinition weapon && !data.isUpgrade)
        {
            var stats = weapon.GetStatsForRarity(ri);
            descriptionText.text = !string.IsNullOrEmpty(weapon.description)
                ? weapon.description
                : CarWeaponStatFormatting.BuildSummary(stats);
        }
        else if (d is CarWeaponDefinition wu && data.isUpgrade)
        {
            var handler = PlayerCarWeaponHandler.Instance;
            ...
            descriptionText.text = !string.IsNullOrEmpty(wu.description)
                ? wu.description
                : CarWeaponStatFormatting.BuildTransitionSummary(prev, next);
        }
