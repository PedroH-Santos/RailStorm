...
- **`Player/Items/PlayerItemHandler.cs`** — `AcquireItem` idempotente; `TryGetStatValue(alvo)` e `TryPreviewStatChange(item, removing, antes, depois)` (valor do stat hoje e depois de ganhar/perder o item, usado no card da loja/venda, 4.16). Aplicar, reverter e a prévia usam as mesmas contas (`WithItem`/`WithoutItem`), então o que a loja mostra é o que acontece; `ApplyStatChange` suporta `MoveSpeed`, `MaxHP`, `HP`, `Coins`, `LuckPercent` (soma ou multiplicador `%`); alvos não tratados só logam warning. `ExileItem`/`IsExiled` (exilado não reaparece em baús, mas ainda pode aparecer na loja — a loja só filtra por `HasItem`). `ResetForNewRun()` limpa tudo.
...
- **DetailCard**: ícone/nome/raridade, descrição, linhas de efeito e bloco "Habilidade Especial" vindos de `TooltipBuilder.Build` (o mesmo conteúdo do tooltip, nada duplicado), e a caixa **Custo / Carteira**.
- **Bônus antes » depois (25/09).** **O que é:** em vez de só "+30%", a linha de ATRIBUTOS mostra quanto o jogador já tem daquele bônus e quanto passa a ter, como o ferreiro faz com as Skills. **Regras:**
  - O valor comparado é o **valor real do stat do jogador**, antes e depois (`PlayerItemHandler.TryPreviewStatChange`), com a mesma conta que a compra/venda aplica. Compra: `Vida Máxima 100 » 130` (item de +30%), `Sorte 0% » 10%` (item de +10). Venda: `130 » 100`.
  - **A unidade é a do atributo, não a do item** (pedido do usuário, 25/09): o mesmo formato do painel Status, via `TooltipBuilder.FormatStatValue` — Vida/Vida Máxima/Moedas inteiros, Velocidade com uma casa (`15,0`), Sorte em `%`. Uma primeira versão mostrava o bônus acumulado dos itens (`+0% » +30%`), depois sem sinal e na unidade do item (`0 » 10` para Sorte); as duas foram reprovadas por não baterem com a medida do atributo. **Sem sinal de `+`**: a cor e a transição já dizem se sobe ou desce.
  - O "depois" é pintado de `improvementColor` (verde, igual ao ferreiro) quando sobe e de `actionDestructive` quando cai (`useThemeLossColor`, com `lossColor` como alternativa), via rich text `<color>` no mesmo formato do `BlacksmithDetailUI`.
  - Quem liga é o chamador: `ShopItemDetailUI.Show(..., itemHandler, removing)` — `ShopUI` passa `removing = false`, `SellUI` passa `true`. Sem handler, a linha volta ao "+30%" simples. Item de `Ability` não é afetado. Custo fica em `actionDestructive` quando não dá para pagar. Com o estoque vazio, mostra "Estoque esgotado".
...
- Blocos do `Content` (espaçamento 10), todos com **altura fixa** por `LayoutElement`, para o orçamento vertical ser previsível:
  ...
  | **`StatsHeader`** (loja/venda) | **52** (25/09, era 36 e a faixa esmagava o texto; mesma geometria do cabeçalho de grupo do Status: título recuado 52px, asas 40×20 a 8px da borda) | faixa `CartoonTitlePlank9Slice` com "ATRIBUTOS", clonada do cabeçalho de grupo do Status, separando a descrição dos números. `ShopItemDetailUI.statsHeader` **esconde a faixa quando o item não tem nenhuma linha de atributo** (item de `Ability` pura, por exemplo). No ferreiro ela fica desativada: não cabe no orçamento |
  | `StatsContainer` | **48** por faixa na loja/venda (25/09, era 38 e o texto encostava na borda), 34 no ferreiro; espaçamento 6 (4 no ferreiro) | o `StatRowTemplate` é uma **faixa** igual às linhas do Status (fundo do `Stats.prefab`, barra de acento cobre de 4px à esquerda no lugar do antigo bullet), rótulo à esquerda e valor à direita (Nunito 22). Loja/venda: padding `22,18,10,10`, spacing 10, valor com `minWidth 180`/`preferredWidth 240` para caber "+0% » +30%" |
...
**Regras:**
- **Layout (25/09) — segue o esboço do usuário:**
...
**Peças de cena (`UI/CanvasHUD`, montado na cena, sem Editor script; posições atuais na tabela de layout acima):** `Counters` (`Coins`, `Kills`: ícone 44px creme com `Outline` + número Lilita 40 cartoon) → `HealthBar` (380×44, fundo `CartoonSlotPlate9Slice` em `#050F1C` + `Shadow`, `Fill` em `#A8392A`, texto Lilita 26) → `BackpackRow/Backpack` (46px, `Button`, filho `KeyBadge` 24px em `CartoonButtonPlate9Slice` Steel com letra Lilita 16). `Timer` e `Wave` dentro do `TopLeft` (tamanhos na tabela de layout). Todos os TMP são **duplicados** do `Value` do `StatsCoins.prefab` (regra de não criar TMP por `AddComponent`, 4.10). Ícones de caveira e mochila são PNGs brancos próprios em `Assets/UI/HUD/` (`HUDSkull.png`, `HUDBackpack.png`, 128², desenhados fora do Editor e editáveis direto); a moeda é o `coin2` do `StatsCoins`.
...
**O que é / ideia central:** a arma do personagem deixou de ter um ataque só. Cada ataque é uma **Skill** (ex.: bola de fogo) que o jogador equipa num **slot com tecla própria** e usa em tempo real, algumas com recarga visível no HUD. O **Ferreiro**, um ponto do mapa (tecla E), é onde o jogador **compra Skills novas**, sobe o nível delas com moedas da run (vendo exatamente quais stats melhoram) e troca quais estão equipadas. Como o jogador pode ter mais Skills do que slots, a tela é dividida em duas abas, LOJA e EQUIPAR (ver "Abas do ferreiro" abaixo). O loadout inicial ainda é decidido **antes da run** (não implementado, ver seção 6).
...
- **O canvas mora na cena, em `UI/CanvasBlacksmith` (25/09),** junto com as outras telas cheias, e começa desativado. Antes ele era filho do prefab, o que duplicaria a tela a cada ferreiro novo. Como prefab não referencia objeto de cena, o `BlacksmithZone.blacksmithUI` fica vazio e o `Open` cai no **`BlacksmithUI.Instance`** (acha o componente com `FindFirstObjectByType(..., Include)`, mesmo inativo). O campo continua servindo de override opcional. Vários ferreiros no mapa compartilham a mesma tela. Mudança visual no ferreiro se faz na cena, não no prefab. Cuidado com o clone `BlackSmithUpgradeSkills(Clone)AnimatorPreview`, que aparece em `FindObjectsOfTypeAll` antes do objeto real.
...
- **Sem cantoneiras de seleção nos slots equipados** (pedido do usuário, 24/09): o `SelectionBrackets` foi removido de lá. A bandeja marrom (`BlacksmithSlotUI.SetSelected`, `selectedPlate`; normal `#0C2238`) agora marca o slot que guarda a **Skill em foco na lista**. Clicar num slot liberado do Inventário abre a aba EQUIPAR com a Skill daquele slot em foco.
...
**O que é / ideia central:** o jogador passa a ganhar Skills durante a run e pode ter mais Skills do que slots. Então o ferreiro precisa vender Skills novas e deixar trocar quais estão equipadas. Como o painel central já estava cheio, ele virou duas abas: **LOJA** (comprar e melhorar) e **EQUIPAR** (escolher o que vai em J/K/L).

**Regras:**
- A tela sempre abre na aba LOJA (`BlacksmithUI.startingTab`). Trocar de aba refaz a lista com a cascata de entrada e mantém o foco na mesma Skill se ela existir na lista nova.
- **LOJA:** primeiro as Skills possuídas (fileira com MELHORAR, como antes), depois as do catálogo que o jogador ainda não tem (fileira com `NOVA · Nv. 1/N`, preço de compra e botão COMPRAR). Sem saldo, o botão fica desativado e o preço em `actionDestructive`. O card de detalhe de uma Skill nova mostra os stats do nível 1 sem seta de melhoria e o preço de compra na caixa Custo/Carteira. A caixa Custo/Carteira só aparece nesta aba.
- **Comprar:** `-N` sobe na Carteira, a fileira vira MELHORAR (e vai para o fim do grupo das possuídas) e, se a Skill caiu num slot vazio, o slot dá o punch de equipar.
- **EQUIPAR:** lista só as possuídas. Cada fileira mostra a tecla onde a Skill está ou "Fora dos slots". No lugar do preço aparece "Equipar em" e, no lugar do botão, **um botão por tecla** (J/K/L, ou o rótulo do gamepad). A tecla onde a Skill já está fica em Copper, as outras em Steel, e as de slot bloqueado ficam desativadas. Clicar numa tecla equipa a Skill ali: se ela estava em outro slot, as duas trocam de lugar; se estava fora dos slots, tira a que ocupava a tecla. O card esconde a caixa Custo/Carteira (`BlacksmithDetailUI.walletBox`) e mostra os stats do nível atual sem seta.
- **Orçamento vertical:** a faixa `Tabs` tem 64px e o `Body` é flexível, então ele encolheu de 760 para 690, e o card de detalhe tem folga nas duas abas.

**Código e cena:**
- **`Blacksmith/UI/BlacksmithTabUI.cs`** — uma aba: `tab`, `button`, `plate`, `label`, `activeColor` (Copper) / `inactiveColor` (Steel) no Inspector, `SetActive(ativa, animar)` troca a cor, esmaece o rótulo inativo e dá um punch. Define também o enum `BlacksmithTab { Shop, Equip }`.
- **`BlacksmithSkillRowUI`** tem três formas de montar (`SetupUpgrade`, `SetupBuy`, `SetupEquip`) e expõe `Mode` (`BlacksmithRowMode { Upgrade, Buy, Equip }`). Em LOJA o botão da fileira chama `BlacksmithUI.OnRowAction`, que decide pela `Mode`. Em EQUIPAR, os botões de tecla ficam em `ActionColumn/SlotButtons` do `BlacksmithRow.prefab` (`slotButtonsRoot`/`slotButtons`, 3 cópias do `UpgradeButton`, HLG spacing 6) e chamam `BlacksmithUI.EquipRowInSlot(fileira, slot)`.
- **`BlacksmithDetailUI.Show(mode, ...)`** decide pelo modo o que aparece (seta de melhoria, carteira).
- Cena: `UI/CanvasBlacksmith/BlacksmithSelectionPanel/BlacksmithPanel/Tabs` (HLG, spacing 12, altura 64), entre `TitleContainer` e `Body`, com `TabShop` ("LOJA") e `TabEquip` ("EQUIPAR"), duplicados do `UpgradeButton` do `BlacksmithRow.prefab` (260×58, Lilita cartoon com autosize 20–30).
...
**Barra de Skills no HUD (`CanvasHUD/SkillBar`, base, à direita da vida, 25/09):**
- **O que é:** as Skills equipadas como **cartas em leque**, escolhidas pelo usuário por mockup (opção I). Sem painel ou moldura atrás.
- **Regras do leque:** as cartas liberadas se abrem em torno do centro. Cada passo de distância do centro inclina `fanAngle = 7°` (a da esquerda gira no sentido anti-horário) e desce `fanDrop = 8px`. Com 1 slot liberado a carta fica reta; com 3, fica `+7° / 0° / −7°`. Quem calcula é `SkillBarUI.ApplyFan`, chamado no fim de todo `Rebuild`. A rotação vai no filho `Card` (`SkillSlotHUD.card`, aplicada por `SkillSlotHUD.SetFan`), não na raiz do slot, porque o punch de escala do cast/pronto anima a raiz e o HLG reposiciona a raiz.
- `UI/HUD/SkillBarUI` fica na `SkillBar` (âncora e pivot `(0,0)`, `anchoredPosition (864, 8)`, HLG com `spacing` 6 e padding `10,10,12,30`, `ContentSizeFitter` preferred nos dois eixos, `fanDrop = 12`). Mostra só os slots liberados e reconstrói em `OnLoadoutChanged`.
- **Anatomia da carta (25/09, segunda passada).** A primeira versão (84×112, creme com janela navy) foi reprovada: "pequena", "o azul não combinou com o branco" e "não está com formato de cartas". O que faz ler como carta é a **proporção 2:3** e a **moldura**. Chegou a ter o nome da skill abaixo da arte, removido a pedido do usuário ("só o ícone já fica legal"). Hoje cada slot tem **112×168**:
  - `Card`: `CartoonCardPlate9Slice` em **Copper** (a moldura), `Shadow (0,-5)`.
  - `Face` (primeiro filho, recuado 7px): `CartoonSlotPlate9Slice` em creme `#FCF8E6`, `ppum 1.5`.
  - `Plate` (90×90, centralizada e 10px acima do centro, para equilibrar com o medalhão embaixo) em **Brown Deep `#663300`** com ícone branco 62px, `CooldownOverlay` (`Image.Type.Filled` **Radial360**, `#150A03` a alpha 0.75), `ReadyFlash` e `CooldownText` (Lilita 40) dentro dela.
