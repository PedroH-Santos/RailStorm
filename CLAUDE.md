...
- `TooltipBuilder` — traduz um `IDrawable` em `TooltipData`, e é o único lugar que sabe o que é "atributo principal" de cada tipo:
  - **Arma**: usa `GetEffectiveStats()` (portanto **já com as `WeaponSkillDefinition` aplicadas**, não os valores base) → Dano, Cadência, Alcance, mais Flechas/Velocidade (`ArrowLevelData`) ou Área/Conjuração (`MagicLevelData`); depois lista cada skill de arma aplicada como uma linha extra.
  - **Habilidade**: efeito (stat alvo + valor) e `Nível atual / total`.
  - **Item**: efeito (`StatChange` → stat alvo + valor; `Ability` → "Habilidade especial").
...
- **Botões da tela de habilidade.** `Exilar`/`Pular`/`Atualizar` têm tamanho fixo por `LayoutElement` (230×76, 250×76 no `Atualizar`) e texto MedievalSharp em 34 — antes herdavam a altura do container e ficavam com ~34px de altura e texto 24, ilegíveis. O `ButtonsContainer` reserva 114px de altura. O contador de usos restantes deixou de ser um texto solto no topo do botão e virou o **badge** `CountBadge` (quadrado escuro arredondado 42×42, ancorado no canto superior direito do botão) com o `CountText` dentro — as referências `exileCountText`/`refreshCountText` do `AbilitySelectionUI` continuam apontando pro mesmo `TMP_Text`, só reparentado.
