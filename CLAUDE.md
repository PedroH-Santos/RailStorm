...
- `ShopPanel` = título "LOJA" → `RefreshTimer` ("NOVOS ITENS EM mm:ss", relógio) → `Body` com `ItemList` (rolável, ~474px) e `DetailCard` (**480px**, ver "Card de detalhe em blocos" abaixo).
- **Uma fileira por item do estoque**, instanciada em runtime de `Assets/Prefabs/UI/ShopRow.prefab` (o `Awake` do `ShopUI` apaga qualquer filho deixado na cena, mesmo padrão de `SellUI`). Fileira: placa com corpo e anel na cor da raridade (mesma fórmula do card de habilidade: `Lerp(panelBackground, Shade(cor, 0.3), 0.7)`), placa de ícone + glow do `RarityConfig`, nome (Lilita) e raridade (Fredoka) e, à direita, a **`ActionColumn`** (150px): preço com moeda em cima e botão **COMPRAR** (52px) embaixo. Altura da fileira: 112. O preço e o botão ficam empilhados (22/09) porque, com a lista mais estreita, lado a lado eles deixavam só ~80px para o nome.
...
- `DetailCard` com `preferredWidth = minWidth = 480` (conteúdo útil ~432). A `ItemList` é flexível e fica com o resto (~474).
- Blocos do `Content` (espaçamento 10), todos com **altura fixa** por `LayoutElement`, para o orçamento vertical ser previsível:
  ...
  |---|---|---|
  | `Header` | 80 | ícone 80 + nome + raridade/nível |
  | `DescriptionBox` | 104 | bandeja `CartoonSlotPlate9Slice` em `slotTray`, padding 16/12, com a `Description` dentro (Fredoka 20, autosize **15–20**, `lineSpacing` 6). Texto longo encolhe em vez de estourar. O `Divider` antigo foi desativado |
  | `StatsContainer` | 36 por faixa, espaçamento 4 | o `StatRowTemplate` é uma **faixa** igual às linhas do Status (fundo do `Stats.prefab`, barra de acento cobre de 4px à esquerda no lugar do antigo bullet, padding 18/12), rótulo à esquerda e valor à direita (Nunito 22, valor com 190px) |
...
- Orçamento do ferreiro (o mais apertado, ~610 úteis): 48 de padding + 80 + 104 + 196 (5 faixas) + 60 + 60 + 50 de espaçamento ≈ 598. Loja e venda têm ~700 e nenhum botão EQUIPAR.
...
**Regras:**
- **Layout (cantos):** sup. esq., de cima para baixo: moedas e inimigos derrotados lado a lado → barra de vida vermelha com `HP / MaxHP` dentro → **mochila** com selo da tecla. Logo abaixo da mochila, o **painel de Skills** (4.19). Topo centro: cronômetro grande. Topo dir.: wave. O centro inferior fica livre de propósito: a barra de skills chegou a ficar lá e foi reprovada porque atrapalhava a visão do jogador.
...
- `SlotsBar` (onde a loja tem o cronômetro): rótulo "SKILLS EQUIPADAS" + 3 `BlacksmithSlotUI` (slot do `WeaponUI.prefab` desempacotado, com `KeyBadge` duplicado da mochila do HUD no topo, cadeado `padlock-locked` e `SelectionBrackets` da fileira). Textos: `LVL n`, "Vazio", "Bloqueado".
...
**Painel de Skills no HUD (`CanvasHUD/TopLeft/SkillsPanel`, canto superior esquerdo, 22/09):**
- **O que é:** uma estrutura própria, pedida pelo usuário para deixar claro que aquela parte do HUD são só as skills. Fica abaixo da mochila, dentro do `VerticalLayoutGroup` do `TopLeft`. A primeira versão, uma barra no centro inferior, foi reprovada por atrapalhar a visão.
- `SkillsPanel` (VLG, `minWidth` 200) = `Header` (cópia do cabeçalho `GroupVital/Header` do Status: faixa `CartoonTitlePlank9Slice` + asas, título "SKILLS" Fredoka 24) + `SkillBar` (bandeja `CartoonSlotPlate9Slice` navy 82% com `Shadow`, HLG centralizado, padding 12/10, altura 92). Hoje o painel acompanha a largura da coluna do `TopLeft` (a da barra de vida).
- `UI/HUD/SkillBarUI` fica na `SkillBar`, com um `SkillSlotHUD` de **72px** por slot. Mostra só os slots liberados e reconstrói em `OnLoadoutChanged`.
- Cada slot: `Plate` (`#0C2238`), ícone branco 46px, `CooldownOverlay` (`Image.Type.Filled` **Radial360**, esvaziando a partir do topo), `ReadyFlash`, `CooldownText` (Lilita 30 cartoon, por cima do radial) e `KeyBadge` de 30px no canto inferior direito.
